...
public class GestureLockLayout extends RelativeLayout {
    ...
    private Path mPath;
    //连接线的宽度
    private float mStrokeWidth = 5.76f;
    //手指触摸是，path颜色
    private int mFingerTouchColor = Color.parseColor("#3CBD23");
    //手指抬起时,密码匹配path颜色
    private int mFingerUpMatchedColor = Color.parseColor("#3CBD23");
    //手指抬起时,密码不匹配path颜色
    private int mFingerUpUnmatchedColor = Color.parseColor("#FF5E4C");
    ...
    private void handleMoveEvent(int x, int y) {
        ...
        if (lockView != null) {
            ...
            if (!mChooseList.contains(childId)) {
                ...
                float a = lockView.getView().getLeft() / 2;
                float b = lockView.getView().getRight() / 2;
    ...
    private boolean checkAnswer() {
        ...
        for (int i = 0; i < mAnswerList.size(); i++) {
            if (mAnswerList.get(i) != mChooseList.get(i)) {
                return false;
            }
        }
    ...
    public void setAnswer(String answer) {
        ...
        for (int i = 0; i < answers.length; i++) {
            mAnswerList.add(Integer.parseInt(answers[i] + ""));
        }
    }
